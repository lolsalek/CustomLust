...
local function BuildBuffSets()
  ...
  if CustomLustDB.debug then
    ...
    for _ in pairs(NS.BUFF_SET_IDS) do idCount = idCount + 1 end
    for _ in pairs(NS.BUFF_SET_NAMES) do nameCount = nameCount + 1 end
...
local function UpdateCountdown()
  if not countdownExpiration or countdownExpiration <= 0 then
    StopCountdown()
    return
  end

  local remaining = countdownExpiration - GetTime()
  if remaining <= 0 then
    StopCountdown()
    return
  end

  -- One decimal like 38.0
  timeText:SetText(string.format("%.1f", remaining))
  timeText:Show()
end

local function StartCountdown(expirationTime)
  countdownExpiration = expirationTime

  -- Some auras can report 0/none; in that case just hide the timer
  ...
  -- Start ticker once
  if not countdownTicker then
    countdownTicker = C_Timer.NewTicker(0.1, UpdateCountdown)
  end
end
...
Effect:SetBackdrop({ edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border", edgeSize = 16 })
...
Effect:SetScript("OnDragStart", function(self)
  if CustomLustDB.locked and not previewActive then return end
  self:StartMoving()
end)
...
local function SetFrameTexCoord(i)
  ...
  if row >= rows then row = row % rows end
...
local function StartEffect(opts)
  opts = opts or {}
  if not CustomLustDB.enabled then return end
 ...
 -- Only run sprite animation and spin when the image overlay is active
  ...
  if not opts.silent then
    pcall(function()
      PlaySoundFile(CustomLustDB.soundPath, CustomLustDB.soundChannel or "Master")
    end)
  end
end
  ...
  if expirationTime and expirationTime > 0 then
    ...
    if remaining > 0 then
      C_Timer.After(remaining + 0.05, function()
        if myId ~= stopTimerId then return end
        local stillHas = select(1, FindActiveTriggerAura())
        if not stillHas then
          active = false
          if not previewActive then StopEffect() end
        end
      end)
    end
  end
...
local function DumpTrackedAuras()
  local idCount, nameCount = 0, 0
  for _ in pairs(NS.BUFF_SET_IDS or {}) do idCount = idCount + 1 end
  for _ in pairs(NS.BUFF_SET_NAMES or {}) do nameCount = nameCount + 1 end
  Print(("Tracking IDs: %d | Tracking names: %d"):format(idCount, nameCount))
